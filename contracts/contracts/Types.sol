...
function deserializeMarketDealNotifyParams(bytes memory rawResp)
    pure
    returns (MarketDealNotifyParams memory ret)
{
    uint byteIdx = 0;
    uint len;
...
function serializeDealProposal(MarketTypes.DealProposal memory dealProposal)
    pure
...
function deserializeDealProposal(bytes memory rawResp)
    pure
    returns (MarketTypes.DealProposal memory ret)
{
    uint byteIdx = 0;
    uint len;
